...
class CartRepository(private val appDatabase : AppDatabase, private val dispatcher: CoroutineDispatcher) {
    ...
    suspend fun deleteCart(cartId: Int) = withContext(dispatcher) { appDatabase.CartDao().delete(cartId) }
    ...
    suspend fun updateAll(cartList: List<Cart>) = withContext(dispatcher) { appDatabase.CartDao().insertAll(cartList) }

    suspend fun updatePosition(carts : List<Cart>) =  withContext(dispatcher) {appDatabase.CartDao().updateOrder(carts)}

    suspend fun deleteAll() : Int = withContext(dispatcher) { appDatabase.CartDao().deleteAll() }
